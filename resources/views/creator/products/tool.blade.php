            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                ...
                <div class="item-card flex flex-row gap-y-10 justify-between md:items-center">
                    <div class="flex flex-row items-center gap-x-3">
                        <img src="{{ Storage::url($product->cover) }}" alt="" class="rounded-2xl object-cover w-[120px] h-[90px]">
                ...
                <form method="POST" action="{{ route("creator.product.tools_store", $product) }}" enctype="multipart/form-data">
                    @csrf

                    <div>
                        <x-input-label for="tools" :value="__('tools')" />
                      
                            <select name="tool_id" id="tool_id" class="py-3 rounded-lg pl-3 w-full border border-slate-300 scroolbar-tool ">
               ...
               <div class="flex flex-row bg-red justify-between">
                   ...
                   <div class="flex flex-row items-center gap-x-3">
                       <form action="{{ route('creator.product.tools_destroy', $tool->pivot->id) }}" method="POST">
                           @csrf
                           @method('DELETE')
